...
const PageSuggestions = ({ onSelectSuggestion }: PageSuggestionsProps) => {
  ...
  return (
    <div className="cdx-px-4 cdx-py-3 cdx-border-b dark:cdx-border-neutral-800 cdx-border-neutral-200">
      {/* 折叠头部 */}
      <button
        type="button"
      ...
      >
        <div className="cdx-flex cdx-items-center cdx-gap-2">
          <RiSparklingFill className="cdx-w-4 cdx-h-4 cdx-text-blue-500" />
          <span className="cdx-text-sm cdx-font-medium cdx-text-neutral-700 dark:cdx-text-neutral-300">
            推荐提示词 {!isLoading && `(${suggestions.length})`}
          </span>
        </div>
        {isExpanded ? (
